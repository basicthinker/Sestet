...
struct inode *rffs_get_inode(struct super_block *sb,
...
/* SMP-safe */
static int
rffs_mknod(struct inode *dir, struct dentry *dentry, int mode, dev_t dev)
{
...
static int rffs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
	return rffs_mknod(dir, dentry, mode | S_IFREG, 0);
}

static int rffs_symlink(struct inode * dir, struct dentry *dentry, const char * symname)
{
	struct inode *inode;
	int error = -ENOSPC;
...
struct dentry *rffs_mount(struct file_system_type *fs_type,
